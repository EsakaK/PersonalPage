...
- 为了充分利用显存，建议大家使用更大的batchsize来跑满显存任务。在任务提交密集时，若存在每张卡利用率不高（低于50%）的任务，会被直接kill，以供需要大显存的任务运行。任务不密集时，不做操作。

- debug卡一般使用G121的gpu0，如果debug时候出现显存被占用并且没有其他debug进程时，可能是因为进入了gpu1.可以通过开启容器之前export CUDA_VISIBLE_DEVICES=0命令，再进入容器。
...
|lx|bit:5000/lx_py3.11_pytorch2.4.1_cu12.1_devel_v2|
|mr|bit:5000/mr_difussion|
|xyn|xyn_py310_torch221_llama3_2:latest|
|xyn|xyn_py310_torch221_llama3:latest|
